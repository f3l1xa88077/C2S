...
static uint8_t s_strip[IMG_MAX_WIDTH * 8 * 1]; // Temporary buffer for reading image strips

// Encodes a grayscale frame into a JPEG in 'out'.
// Streams the frame one band at a time so peak RAM is a single band
// src: pointer to the source frame in memory
// width, height: dimensions of the frame
// quality: JPEGE_Q_BEST/_HIGH/_MEDIUM/_LOW/_WOR
                    ...
                    uint32_t *out_len)
{
    ...
    *out_len = 0;
    ...
    rc = JPEGEncodeBegin(&jpeg, &enc, (int)width, (int)height,
                         JPEGE_PIXEL_GRAYSCALE, JPEGE_SUBSAMPLE_444, quality);
    if (rc != JPEGE_SUCCESS)
        return rc;
    ...
    pitch       = (uint32_t)width; // Bytes per row for grayscale
    ...
    for (row = 0u; row < (uint32_t)height; row += strip_rows) {
        ...
        for (mx = 0; mx < jpeg.iMCUWidth; mx++) {
            ...
            uint8_t *p = s_strip + (uint32_t)mx * (uint32_t)enc.cx;
