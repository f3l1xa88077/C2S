...
#define ICER_BITMASK_MACRO(x) (((x) >= 32u) ? 0xFFFFFFFFu : (((1u << (x)) - 1u)))
...
int icer_get_bit_from_codeword(icer_decoder_context_typedef *decoder_context, uint8_t bits) {
    ...
    if (decoder_context->decoded_bits_total + bits > decoder_context->encoded_bits_total) {
    ...
    return (decoder_context->encoded_words[ind] >> bitoffset) & 0x01u;
}
...
int icer_get_bits_from_codeword(icer_decoder_context_typedef *decoder_context, uint8_t bits) {
    ...
    while (bits) {
        ...
        num |= (int)((((decoder_context->encoded_words[ind] >> bitoffset) & ICER_BITMASK_MACRO(bits_to_decode))) << decoded);
        bits -= bits_to_decode;
...
int icer_pop_bits_from_codeword(icer_decoder_context_typedef *decoder_context, uint8_t bits) {
    ...
    while (bits) {
        ...
        bits -= bits_to_decode;
        decoded += bits_to_decode;
        decoder_context->decoded_bits_total += bits_to_decode;
        bitoffset += bits_to_decode;
        if (bitoffset >= 8) {
            bitoffset -= 8;
            ind++;
    ...
    decoder_context->encode_bit_offset = bitoffset;
    decoder_context->encode_ind = ind;
    return num;
}
...
int icer_decode_bit(icer_decoder_context_typedef *decoder_context, uint8_t *bit, uint32_t zero_cnt, uint32_t total_cnt) {
    ...
    b = (decoder_context->bin_buf[bin][bin_ind] & (1u << (bit_offset - 1))) != 0;
    decoder_context->bin_buf[bin][bin_ind] &= ~(1u << (bit_offset - 1));
